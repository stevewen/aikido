...
{
  mLookahead = _lookahead;
  std::cout << "Lookahead set at: " << mLookahead << std::endl;
}

void LRAstar::setGreediness(double _greediness)
{
  mGreediness = _greediness;
  std::cout << "Greediness set at: " << mGreediness << std::endl;
}
...
std::vector<Vertex> LRAstar::pathToBorder(Vertex _vertex)
{
  std::vector<Vertex> path;
  path.emplace_back(_vertex);
  double currentBudget = g[_vertex].node.budget();

  while(currentBudget > 0)
  {
    _vertex = g[_vertex].node.parent();
    assert(g[_vertex].node.budget() == currentBudget - 1);
    path.emplace_back(_vertex);
    currentBudget = g[_vertex].node.budget();

    assert(g[_vertex].visited);
    assert(g[_vertex].node.parent() != _vertex);
    assert(g[_vertex].node.budget() <= mLookahead);
  }
  return path;
}

double LRAstar::estimateCostToCome(Vertex _vertex)
{
  return g[_vertex].node.cost() + g[_vertex].node.lazyCost();
}

double LRAstar::heuristicFunction(Vertex _vertex)
{
  return mSpace->distance(g[_vertex].v_state->state, g[mGoalVertex].v_state->state);
}

double LRAstar::estimateTotalCost(Vertex _vertex)
{
  return estimateCostToCome(_vertex) + heuristicFunction(_vertex);
}
    ...
    else
    {
      ...
      for (boost::tie(ni, ni_end) = adjacent_vertices(u, g); ni != ni_end; ++ni)
      {
        ...
        if(g[uv].edgeStatus == CollisionStatus::FREE)
        {
          ...
          {
            // TODO: Threshold set based on numerical precision
            ...
            // TODO(Avk): make function to remove children
            ...
            //TODO: Clear the subtree even if budget is less than alpha!! But later you can just update them
...
void LRAstar::rewireLazyBand(TG &qRewire, TF &qExtend, TF &qFrontier)
{
  ...
  while(!qRewire.empty())
  {
    ...
    for (boost::tie(ni, ni_end) = adjacent_vertices(u, g); ni != ni_end; ++ni)
    {
      ...
      assert(v != p); // Enforce prevention of loops - parent should not ne in Qrewire
...
bool LRAstar::evaluatePath(std::vector<Vertex> path, TG &qUpdate, TG &qRewire, TF &qExtend, TF &qFrontier)
{
  ...
  for(auto iterV = path.rbegin(); iterV != path.rbegin() + greediness && iterV != path.rend(); ++iterV)
  {
    ...
    Node nu = g[*iterV].node;
    Node nv = g[*(iterV + 1)].node;
    ...
    else
    {
      ...
      g[v].node.updateBudget(0);
      g[v].node.updateLazyCost(0);
      g[v].node.updateCost(nu.cost() + edgeLength);
...
void LRAstar::setProblemDefinition(const ::ompl::base::ProblemDefinitionPtr &pdef)
{
  ...
  // Snap Start and Goal to nearest vertices
  // TODO (AVK): Add them to roadmap and connect them to K-Nearest Neighbors
  double bestStartDist = std::numeric_limits<double>::max();
  double bestGoalDist  = std::numeric_limits<double>::max();
...
{
  auto cmpG = [&](Vertex left, Vertex right)
  {
  ...
  auto cmpF = [&](Vertex left, Vertex right)
  {
  ...
  std::set<Vertex, decltype(cmpG)> qUpdate(cmpG);
  std::set<Vertex, decltype(cmpG)> qRewire(cmpG);
  std::set<Vertex, decltype(cmpF)> qExtend(cmpF);
  std::set<Vertex, decltype(cmpF)> qFrontier(cmpF);
  ...
  assert(ptc == true);
  std::cout << "Planning Termination Condition Satisfied" << std::endl;
